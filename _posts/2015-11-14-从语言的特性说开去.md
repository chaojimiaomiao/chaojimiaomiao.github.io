...
所以，就__语言层面__来说，OC因为其动态的特性，比如一个selector(相当于method)究竟存不存在，在编译阶段我们不得而知；就__内存管理__来说，OC没有像Java一样时刻不停监控内存、回收内存的管理器；就__人员角度__来说，自ARC以后讨论OC内存泄露的技术文章少之又少。 __<font color="blue">不管如何看，OC都比Java更可能发生内存泄露。</font>__
